    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-[640px] h-[480px] bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="space-y-6">
                <div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between"><span className="text-xs text-slate-600">默认导出格式</span><select className="text-xs border border-slate-200 rounded p-1.5 bg-slate-50 outline-none focus:border-indigo-500"><option>Word 文档 (.docx)</option><option>PDF 文件 (.pdf)</option></select></div>
                        <div className="flex items-center justify-between"><span className="text-xs text-slate-600">自动保存间隔</span><select className="text-xs border border-slate-200 rounded p-1.5 bg-slate-50 outline-none focus:border-indigo-500"><option>实时</option><option>每 30 秒</option><option>每 5 分钟</option></select></div>
                    </div>
                </div>
              </div>
            )}
                ...
                <div>
                    ...
                    {user ? (
                        <div className="p-4 bg-indigo-50/50 border border-indigo-100 rounded-lg">
                            <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold">{user.email?.[0].toUpperCase()}</div><div><p className="text-sm font-bold text-slate-900">{user.email}</p><p className="text-xs text-slate-500">LawLens 普通用户</p></div></div>
                        </div>
                    ) : (<div className="text-center py-8 bg-slate-50 rounded-lg border border-dashed border-slate-200"><p className="text-xs text-slate-500 mb-3">您尚未登录</p><Link href="/auth"><Button size="sm" className="bg-slate-900 text-white h-8 text-xs">去登录</Button></Link></div>)}
                </div>
              </div>
            )}
            {activeTab === 'ai' && (
              <div className="space-y-6">
                <div>
                    ...
                    <div className="p-3 bg-blue-50 text-blue-700 text-xs rounded-md border border-blue-100 mb-4">当前正在使用 <b>LawLens Pro (Qwen-Max)</b> 引擎，已开启 RAG。</div>
                    <div className="space-y-3"><label className="flex items-center gap-2"><input type="checkbox" checked readOnly className="rounded text-indigo-600"/><span className="text-xs text-slate-700">启用思维链 (CoT)</span></label><label className="flex items-center gap-2"><input type="checkbox" checked readOnly className="rounded text-indigo-600"/><span className="text-xs text-slate-700">启用 Agent Memory (记忆)</span></label></div>
                </div>
              </div>
            )}
            {activeTab === 'theme' && (
              <div className="space-y-6">
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       <div className="border-2 border-indigo-600 rounded-lg p-3 bg-white text-center cursor-pointer"><div className="text-xs font-bold text-indigo-700">专业白</div></div>
                       <div className="border border-slate-200 rounded-lg p-3 bg-slate-900 text-center opacity-60"><div className="text-xs font-bold text-white">深邃黑 (开发中)</div></div>
                    </div>
                </div>
              </div>
            )}
        ...
        <div className="p-4 border-t border-slate-100 flex justify-end gap-2 bg-slate-50">
           <button onClick={onClose} className="px-4 py-2 bg-white border border-slate-200 rounded-md text-xs font-medium text-slate-600 hover:bg-slate-100">取消</button>
           <button onClick={() => { onClose(); alert("设置已保存！") }} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-xs font-medium hover:bg-indigo-700">保存</button>
        </div>
      </div>
    </div>
  )
...
function MainContent() {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true)
    const fileName = `LawLens_文书_${new Date().toISOString().slice(0,10)}.docx`
  ...
  const fillTemplate = (id: string) => {
    ...
    if (template) {
      ...
      setSaveStatus('saved')
      setMode('polish') 
      setPolishMessages(prev => [...prev, { role: 'assistant', content: `已加载【${template.title}】。` }])
  ...
  const handleRiskScan = async () => {
    if (!content || content.length < 50) { showToast("内容太少，无法分析", "error"); return }
    ...
    } catch (e) { showToast("分析失败", "error") } finally { setIsRiskScanning(false) }
  }
      ...
      <div className={cn(
         ...
         <div className="h-16 px-5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
            ...
            <button onClick={handleRiskScan} disabled={isRiskScanning} className="flex items-center gap-1 bg-indigo-50 text-indigo-600 px-2 py-1 rounded text-[10px] font-bold hover:bg-indigo-100 transition-colors disabled:opacity-50">
                {isRiskScanning ? <Loader2 className="animate-spin w-3 h-3"/> : <Activity className="w-3 h-3" />} 体检
            </button>
         ...
         <div className="flex-1 overflow-y-auto p-5 bg-[#FAFAFA] scroll-smooth relative">
            ...
            {messages.length === 0 && !riskData && (
                <div className="mt-8 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-slate-900 font-bold mb-2 text-sm">{mode === 'draft' ? 'AI 智能起草' : 'AI 审查润色'}</h3>
                    <p className="text-xs text-slate-500 mb-6 px-8 leading-relaxed">
                        {mode === 'draft' ? '描述您的需求，AI 将为您从零起草。' : '文档已加载。尝试点击上方「体检」按钮。'}
                    </p>
                    <div className="space-y-2.5 px-2">
                        {(mode === 'draft' ? ['起草一份租赁合同', '写一份律师函'] : ['优化选中条款', '检查法律风险']).map((t, i) => (
            ...
            <div className="space-y-6">
                ...
                {isAnalyzing && (<div className="flex gap-2 items-center text-xs text-slate-400 px-2 mt-2"><Sparkles className="w-3.5 h-3.5 animate-pulse text-indigo-500" /> <span className="animate-pulse">AI 正在思考...</span></div>)}
                <div ref={chatEndRef} />
            </div>
         </div>
         ...
         <div className="p-4 bg-white border-t border-slate-100">
            ...
            <div className="relative group">
                <textarea 
                    ...
                    placeholder={chatWithDoc ? "针对本文档提问..." : (mode === 'draft' ? "描述案情以起草..." : "输入修改指令...")} 
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden bg-[#F2F4F7]">
         <header className="h-16 flex items-center justify-between px-8 bg-[#F2F4F7] shrink-0">
             ...
             <div className="flex items-center gap-2">
                 <Button variant="outline" size="sm" onClick={handleExport} disabled={isExporting} className="h-8 text-slate-600 border-slate-200 hover:bg-white text-xs shadow-sm bg-white">{isExporting ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-2"/> : <Download className="w-3.5 h-3.5 mr-2" />} 导出 Word</Button>
                 <Button size="sm" onClick={handleShare} className="bg-[#1A1A1A] hover:bg-black text-white h-8 px-4 rounded-md shadow-lg shadow-black/10 font-medium text-xs flex items-center gap-2"><Share2 size={12} /> 分享</Button>
             </div>
         </header>
          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: "spring", stiffness: 350, damping: 35 }} className="fixed inset-y-0 right-0 w-[340px] bg-white shadow-2xl z-[70] flex flex-col border-l border-slate-100">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {!user ? (
                    <div className="text-center py-10"><Lock className="w-8 h-8 text-slate-300 mx-auto mb-2" /><p className="text-xs text-slate-500 mb-4">请登录后查看云端历史</p><Link href="/auth"><Button size="sm" variant="outline" className="text-xs">去登录</Button></Link></div>
